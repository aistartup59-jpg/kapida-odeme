...
| 24 | newest | Return the full PaymentRequestResponseDto from recordTransaction (was raw entity) | `4b851d5` | Payment / Transaction | Bug Fix | Fixed |
...
**Status:** Phase 1 (Authentication), Phase 2 (Merchant Provider), Phase 3 (Payment Creation), and Phase 4 (Payment Lifecycle) complete — 21 spec files, 120 tests, all passing against `postgres-test`. Phase 5 (Transaction) next.
...
**Current task:** Backend Freeze in effect (see Backend Freeze Rules above). Backend Integration Test Suite: Phase 1 (Authentication, 8 files, 42 tests), Phase 2 (Merchant Provider, 4 files, 19 tests), Phase 3 (Payment Creation, 4 files, 25 tests), and Phase 4 (Payment Lifecycle, 5 files, 34 tests) all complete and passing — 21 suites / 120 tests total. Two bugs found and fixed during testing so far: Phase 1's cross-tenant employee creation (#23) and Phase 4's `recordTransaction` response missing `remainingAmount`/`transactions` (#24). Phases 2 and 3 found no bugs.

**Next task:** Phase 5 (Transaction) integration tests: Cash, NFC, QR, Payment Link, Multiple Transactions, Hybrid Payment, Remaining Amount, paidAmount, paidAt — under `backend/test/integration/transaction/`.
...
- Next: Phase 5 (Transaction) integration tests — Cash, NFC, QR, Payment Link, Multiple Transactions, Hybrid Payment, Remaining Amount, paidAmount, paidAt.
