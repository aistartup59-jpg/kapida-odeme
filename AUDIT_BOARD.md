...
**Status:** Phases 1–7 complete — 30 spec files, 170 tests, all passing against `postgres-test`. Phase 8 (Security) next.
...
**Current task:** Backend Freeze in effect (see Backend Freeze Rules above). Backend Integration Test Suite: Phases 1–7 all complete and passing — 30 suites / 170 tests total (Phase 1: 8/42, Phase 2: 4/19+1, Phase 3: 4/25, Phase 4: 5/34, Phase 5: 4/23, Phase 6: 4/19, Phase 7: 1/6). Four bugs found and fixed during testing so far: Phase 1's cross-tenant employee creation (#23), Phase 4's `recordTransaction` response missing `remainingAmount`/`transactions` (#24), Phase 6's `PAYMENT_LINK` create response missing `linkUrl`/`linkExpiresAt` (#25), and Phase 7's malformed payment/provider id causing a raw `500` instead of `400` (#26). Phases 2, 3, and 5 found no bugs.

**Next task:** Phase 8 (Security) integration tests: Unauthorized, Wrong Merchant, Wrong Employee, JWT, Refresh, Expired Token, Forbidden — under `backend/test/integration/security/`. Much of this ground is already covered incidentally by earlier phases (`authorization.spec.ts`, ownership tests throughout); Phase 8 should consolidate/fill gaps rather than duplicate.
...
- Next: Phase 8 (Security) integration tests — Unauthorized, Wrong Merchant, Wrong Employee, JWT, Refresh, Expired Token, Forbidden. Much of this is already incidentally covered by earlier phases; Phase 8 should focus on filling real gaps, not re-testing what `authorization.spec.ts` and the various ownership tests already established.
