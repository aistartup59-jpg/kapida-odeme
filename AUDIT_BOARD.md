...
| 25 | newest | Thread PAYMENT_LINK's linkUrl/linkExpiresAt into the create response (was discarded) | `5200bd1` | Payment / Payment Provider | Bug Fix | Fixed |
...
**Status:** Phases 1–6 complete — 29 spec files, 162 tests, all passing against `postgres-test`. Phase 7 (Cancellation) next.
...
**Current task:** Backend Freeze in effect (see Backend Freeze Rules above). Backend Integration Test Suite: Phases 1–6 all complete and passing — 29 suites / 162 tests total (Phase 1: 8/42, Phase 2: 4/19, Phase 3: 4/25, Phase 4: 5/34, Phase 5: 4/23, Phase 6: 4/19). Three bugs found and fixed during testing so far: Phase 1's cross-tenant employee creation (#23), Phase 4's `recordTransaction` response missing `remainingAmount`/`transactions` (#24), and Phase 6's `PAYMENT_LINK` create response missing `linkUrl`/`linkExpiresAt` (#25). Phases 2, 3, and 5 found no bugs.

**Next task:** Phase 7 (Cancellation) integration tests: Cancel, Cancel after Paid, Cancel after Partial, Invalid Cancel — under `backend/test/integration/lifecycle/` or a new `test/integration/cancellation/` (baseline coverage already exists in Phase 4's `cancelled.spec.ts`; Phase 7 goes deeper on policy edge cases per the original scoping note).
...
- Next: Phase 7 (Cancellation) integration tests — Cancel, Cancel after Paid, Cancel after Partial, Invalid Cancel (deeper policy edge cases beyond Phase 4's baseline `cancelled.spec.ts` coverage).
