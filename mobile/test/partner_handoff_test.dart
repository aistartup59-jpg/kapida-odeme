...
import 'package:kapida_odeme_mobile/core/partner_handoff.dart';
  ...
  group('PartnerHandoff.parse', () {
    test('reads a complete hand-off link', () {
      final handoff = PartnerHandoff.parse(
        Uri.parse('kapidaodeme://collect?token=hof_abc123_def456&returnUrl=partner://order/4471'),
      );
    ...
    test('treats a missing returnUrl as no automatic return', () {
      final handoff = PartnerHandoff.parse(Uri.parse('kapidaodeme://collect?token=hof_abc123_def456'));
    ...
    test('rejects a link for another scheme or host', () {
      ...
      expect(PartnerHandoff.parse(Uri.parse('kapidaodeme://settings?token=hof_a_b')), isNull);
    });
    ...
    test('rejects a missing or blank token', () {
      ...
      expect(PartnerHandoff.parse(Uri.parse('kapidaodeme://collect')), isNull);
      expect(PartnerHandoff.parse(Uri.parse('kapidaodeme://collect?token=')), isNull);
      expect(PartnerHandoff.parse(Uri.parse('kapidaodeme://collect?token=%20')), isNull);
    });
    ...
    test('takes nothing but the token from a link that also carries an amount', () {
      ...
      final withAmount = PartnerHandoff.parse(
        Uri.parse('kapidaodeme://collect?token=hof_abc_def&amount=999999&merchantId=someone-else'),
      );
      final withoutAmount = PartnerHandoff.parse(Uri.parse('kapidaodeme://collect?token=hof_abc_def'));
